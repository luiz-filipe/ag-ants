...
public class AntAgentFactory {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(AntAgentFactory.class);

	/**
	 * Sometimes it is necessary to have a reasonable large number of agents to
	 * use in simulations. This method helps users to create populations of ants
	 * of the Worker type. Note that the entire population will be placed at the
	 * same node.
	 * 
	 * @param numberOfAgents
	 *            The size of the population to be generated
	 * @param namePrefix
	 *            Each agent will receive a id that starts with the prefix plus
	 *            a number, e.g. 'prefix-01'.
	 * @param initialNode
	 *            The node the population is going to be placed at.
	...
	public static List<AntAgent> produceBunchOfWorkers(
		...
		for (int i = 0; i < numberOfAgents; i++) {
			agents.add(new WorkerAntAgent(namePrefix + "-" + i, initialNode,
					false));
		}

		logger.debug("{} agents created with prefix '{}'", agents.size(),
				namePrefix);
		return agents;
	}
	 ...
	 * @param namePrefix
	 *            namePrefix Each agent will receive a id that starts with the
	 *            prefix plus a number, e.g. 'prefix-01'.
	 * @param grid
	 *            Node grid
	 * @param line
	 *            The index of the line of the grid that the population will be
	 *            placed at
	 * @param startColumn
	 *            The index of the column that the population will start to be
	 *            placed.
	 * @param horizontalSpacing
	 *            The number of nodes that separates the chucks of the
	 *            population
	 * @param numberOfAgentsPerNode
	 *            Number of agents at each chuck
	...
	public static List<AntAgent> produceBunchOfWorkers(
		...
		//
		// index of last column needed = startColumn + numberOfColumnsNeeded -
		// 1;
		final int numberOfColumnsNeeded = interations * (1 + horizontalSpacing)
				- horizontalSpacing;
		final int lastColumnIndexNeeded = startColumn + numberOfColumnsNeeded
				- 1;
		...
		try {
			...
			throw new RuntimeException(
					"The grid has not enough cloumns to create the population! Index of last column needed ="
							+ lastColumnIndexNeeded);
		}

		for (int i = 0; i < interations; i++) {
			for (int j = 0; j < numberOfAgentsPerNode; j++) {
				agents.add(new WorkerAntAgent(
						namePrefix + "-" + nAgentsCreated, grid[line][colunm],
						false));
				nAgentsCreated++;
			}
		...
		logger.info("{} agents created with prefix '{}'", agents.size(),
				namePrefix);
		return agents;
	}
}
