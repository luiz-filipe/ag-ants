...
/**
 * The Forage task implements a node selection model to direct the ant towards a food source using pheromone trails.
 * This model takes in consideration the intensity of forage pheromone present in the 8 neighbour nodes around the  agent.
 *
 *  <p>Firstly the pheromone intensities of the neighbour nodes are read. After that the function
 *  <em>exp(intensity * coefficient)</em> is applied for each of the neighbour nodes. The results from this step are
 *  used to calculate the probability of each neighbour node of being selected.</p>
 *
 *  <p>Note that the coefficient is an arbitrary number used to enhance the probability of the nodes with pheromone in it.
 *  See  <em>model/Node Selection.xlsx</em> for more details.</p>
 *
...
public class ForageTask extends AbstractTask {
    ...
    /**
     * Selects one neighbour node to move to, taking in consideration the amount of pheromone present in the environment
     * around the agent.
     *
     * <p>Note that this implementation does not take in consideration the direction the ant is travelling. This could
     * be important otherwise the ant might get stuck going back and forth for no reason.</p>
     *
     * <p>Also remember that we have 8 possible directions to move to.</p>
     *
    ...
    /**
     * Utility method to return the intensity of the <i>ForageChemicalStimulusType</i> present in a node in the
     * neighbouring node in a given direction.
     *
     * @param direction direction of the neighbour node that will have the stimulus' intensity inferred.
     * @param ant ant agent.
     * @return intensity of stimulus or zero if no neighbour is found at the selected direction.
     */
    private static double getForageIntensity(Direction direction, Agent ant) {
        final PheromoneNode neighbour = (PheromoneNode) ant.getCurrentNode().getNeighbour(direction);

        if (neighbour == null) {
            return 0;
        }

        final ChemicalCommStimulus stimulus = neighbour.getCommunicationStimulus(ForageStimulusType.TYPE);
